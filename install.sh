...
ln -sf "$DOTFILES_DIR/schedule.sh" "$CONFIG_DIR/schedule.sh"
ln -sf "$DOTFILES_DIR/window.sh" "$CONFIG_DIR/window.sh"
ln -sf "$DOTFILES_DIR/assets" "$CONFIG_DIR/assets"

# Make scripts executable
echo "🔑 Setting executable permissions for scripts..."
chmod +x "$CONFIG_DIR/media.sh" "$CONFIG_DIR/weather.sh" "$CONFIG_DIR/vitals.sh" "$CONFIG_DIR/schedule.sh" "$CONFIG_DIR/window.sh"
